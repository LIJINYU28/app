<template>
  <div>
    ...
    -->
    <el-dialog
      :title="`${this.formTrademark.id?'修改':'添加'}品牌`"
...
export default {
  ...
  data() {
    return {
      ...
      rules: {
        tmName: [
          // required：必须的  trigger：触发器，什么时候进行表单验证
  ...
  methods: {
    ...
    isShowAdd() {
      ...
      // 当这个添加品牌的对话框出现时，先把所有的表单验证的提示信息清理掉
      // 当前让对话框渲染出来，但是里面的DOM元素还未渲染完毕，此时直接获取ruleForm获取不到。通过$nextTick获取
      this.$nextTick(() => {
